...
const KNOWN_LIMITATIONS: KnownLimitation[] = [
  ...
  {
    description:
      "Real login (email/password on /login) still does not reach a working staff/student view in the dev server, even though the session cookie itself is now set correctly. OPEN — do not show any UI to Alliance until this is fixed.",
    foundIn:
      "src/app/[locale]/login/actions.ts (the now-fixed half); src/lib/tenant/context.ts (getTenantContext/requireTenantContext, the still-open half); src/auth.config.ts (the jwt callback that sets activeOrganizationId: null on every sign-in)",
    dueBy: "Before Phase 4/5 UI work is shown to Alliance — both phases are almost entirely UI and need a working login to demo",
    reason:
      "Two distinct bugs stacked, discovered in this order. (1) FIXED: login() called signIn(\"credentials\", { redirect: false }) then did its own manual redirect() afterward. Per next-auth v5's own documented pattern (confirmed via Context7 against /nextauthjs/next-auth) and confirmed empirically (the login POST had zero Set-Cookie despite a successful authorize(), and the follow-up request had zero Cookie header), redirect:false does not reliably let next-auth attach the session cookie inside a Server Action. Fixed by removing redirect:false and passing the resolved destination as signIn's own redirectTo, letting signIn's internal redirect carry the cookie — verified via a direct curl round-trip (POST captures Set-Cookie, a follow-up GET with that exact cookie is accepted by src/middleware.ts's auth() wrapper, decoding the correct user id/role every time). (2) STILL OPEN: even with a perfectly valid, correctly-decoding session, every staff page redirects straight back to /login, because requireTenantContext() (src/lib/tenant/context.ts) treats a null session.activeOrganizationId as NO_MEMBERSHIP and unconditionally redirects to /login — and activeOrganizationId is null on EVERY fresh sign-in (src/auth.config.ts's jwt callback sets it to null whenever `user` is present, i.e. on every login) with nothing in the app today that ever sets it to a real value except unstable_update({activeOrganizationId}), which has no caller yet (Phase 5's org switcher, not yet built — the 1f-era switchActiveOrganization prototype was removed pending that real UI). So every real login for every user currently ends at /login regardless of role, and this is indistinguishable from the outside from \"the cookie doesn't work\" — that is almost certainly why this looked like a cookie bug across Phases 2d, 3b, and 3c-i's blocked screenshots. Candidate fix: on login (or on first request after login), auto-select the user's sole OrganizationMembership when they have exactly one, falling back to an org picker only when they belong to more than one — cheaper than waiting for the full Phase 5 switcher and unblocks every single-org user (which is 100% of today's seeded QA users). src/app/api/e2e-auth-bypass/route.ts (dev-only, screenshot-only) works around this by resolving a real membership and running the jwt callback's own trigger:\"update\" path to set activeOrganizationId before minting the cookie — a workaround for verification only, not a fix for real login.",
  },
];

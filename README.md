...
| `CRON_SECRET` | Always | Shared secret Vercel Cron sends as `Authorization: Bearer <value>` to authenticate the scheduled job below. Also gates `GET /api/health/jobs` (job freshness — see below). Any long random value. |
| `HEALTHCHECK_DIGEST_URL` | Production, once Healthchecks.io is set up (`docs/DEPLOYMENT_RUNBOOK.md` "Set up the Healthchecks.io dead-man's switch") | The cron's own Healthchecks.io ping URL — a plain GET means success, `<url>/fail` means failure. Unset in dev/CI on purpose: a missing URL is reported as `heartbeatStatus: "not_configured"` on the `JobRun` row (visible via `/api/health/jobs`), never silently treated as success. |
...
One scheduled job runs on Vercel Cron (`vercel.json`), authenticated by `CRON_SECRET`. There is no promotion job: promotions are always awarded by an instructor.

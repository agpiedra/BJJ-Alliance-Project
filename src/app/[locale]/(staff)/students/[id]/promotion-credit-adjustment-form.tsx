...
import { adjustPromotionCredit } from "./promotion-credit-actions";
import type { ActionState } from "@/lib/action-state";

const INITIAL_STATE: ActionState = {};

// Same visibility as AddAdjustmentForm — no page.tsx role gate; the
// server-side adjustPromotionCredit (ADMIN/DIRECTOR/INSTRUCTOR) is the real
// enforcement.
export function PromotionCreditAdjustmentForm({
  organizationId,
  studentId,
}: {
  ...
  const t = useTranslations("students.detail.creditAdjustment");
  const [state, formAction, isPending] = useActionState(
    adjustPromotionCredit.bind(null, organizationId),
    INITIAL_STATE,
  );
    ...
    <details className="rounded border p-4">
      ...
      <form action={formAction} className="mt-4 flex w-full max-w-sm flex-col gap-3">
        ...
        <label className="flex flex-col gap-1">
          <span>{t("delta")}</span>
          <input type="number" name="delta" step="1" required className="rounded border px-3 py-2" />
          <span className="text-sm text-muted-foreground">{t("deltaHint")}</span>
        </label>
        {deltaErrors && deltaErrors.length > 0 && <p className="text-sm text-red-600">{t("deltaInvalid")}</p>}

...
- **Nothing can reverse an archive**, though the confirmation says "This can be reversed by editing status later." `status` is deliberately not editable on the edit form, so that promise has nothing behind it. A future un-archive must go through `grantStudentMembership`, which already switches a deactivated `STUDENT` membership back on.
- A **pending applicant** who logs in sees the generic "your account isn't linked to any organization" — accurate, but it does not say "awaiting approval".
...
management** was closed by revision 35. The version of this paragraph before revision 33

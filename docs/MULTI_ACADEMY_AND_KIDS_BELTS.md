...
2. **The tenant guard blocks `paymentPlan.update` without `organizationId`** — every plan edit, deactivate and reactivate would have thrown in production. Caught by the integration test, not by the browser; fixed at both call sites.
...
**Known limits, deliberately not fixed here.** The system promo plan's name is Spanish in every organization (an English academy sees `Promoción personalizada`); it is a code-level constant the promo flow keys on. On the student page the payment form lists only *active* plans, so correcting a payment that sits on a deactivated plan *from that page* means moving it to an active plan (done in the verification); the Pagos edit sheet is written to keep such a payment's own plan selectable, but that path has neither a test nor a browser run yet. Currency is **not** the same gap as timezone: an organization outside `America/Costa_Rica` is still wrong from day one on attendance dates and check-in windows (launch checklist).
...
| **No staff management** (nothing adds staff, assigns them to academies, or removes them — only the seed writes `staffAssignment`; found in revision 33) | Alliance cannot add a single instructor, and a per-location director can never be appointed, even with the owner role fixed. Deactivation is impossible per organization too: `User.active` is global, so it would lock someone out of every organization they belong to. | **Alliance's launch** — its real staff need accounts. Its own PR (B2): Owner-only invite, role, academy assignment and deactivate, with `OrganizationMembership.active`. |
| **`acceptInvitation` overwrites an existing account's password** (found in revision 33) | Someone who already has an account, invited into a second organization, silently loses their password. Reachable today at bootstrap: runbook step 9 emails a redundant invitation to the platform admin's own address — **do not open that link**. | **Any invited existing user**, which includes the platform admin at bootstrap. Fixed in B2 with its own failing test: an existing account keeps its password and gains the membership; only a brand-new account sets one. |
...
sending domain, and (both found in revision 33, by driving the product as a registered owner
instead of a seeded user) **no way to create a second location** and **no staff management**.
The earlier version of this paragraph said exactly one item blocked Alliance; it was right
about everything anyone had looked at, and wrong because nobody had tried to set Alliance up
through the real flow. One (terms of service) is a business decision this doc can surface but

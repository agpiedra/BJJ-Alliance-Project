...
| **The kiosk check-in screen shows "NaN"** (found in revision 38, in a real browser; the fourteenth "built it, never wired it" instance) | Every real check-in, at every organization, shows "1 / NaN" and "NaN attendances to go for your next stripe": `kiosk-client.tsx` reads `remainingToNextStripe` and `examEligible`, the API returns `remainingAttendance` and `isEligible`. The check-in itself is saved correctly; the screen a student stands in front of is wrong. The portal's self-check-in reads the right names. | **Alliance's launch** — the one screen every student sees. Its own small PR: read the real field names, with a test that renders the success screen from a real `AtBeltSummary`. |
...
**Read together:** three items above block Alliance's own launch outright — the Resend
sending domain, and (found by driving the product as a registered owner or a registered
student instead of a seeded user) **a student account cannot also be staff** (B4, found in revision 35 and reclassified from
"will be hit" to a blocker in revision 36), and **the kiosk check-in screen shows "NaN"**
(found in revision 38). Closed, and no longer counted: **no way to create a second location**
(B3 — found in revision 33, built in revision 38, merged as #46) and **no real
student could reach the portal** (B0 — found in revision 36, closed by revision 37). **No staff

...
**The availability rule - CONFIRMED by the owner, for every class: one shared rule, computed per class.** `opensAt` = the class's scheduled start minus 30 minutes; `closesAt` = its scheduled start plus **its own configured duration** plus 30 minutes; **both boundaries inclusive**; America/Costa_Rica (UTC-6, no daylight saving) and each class's own duration. There is **no shared evening window and no fixed cutoff** (no "9 pm"). Examples: an 18:00-19:00 class is open 17:30-19:30; a 19:00-20:30 class 18:30-21:00; an 18:30-19:30 class 18:00-20:00. *History, so this is not misread: an earlier draft of this PR proposed start minus 30 to start plus 30 minutes and described it as owner-approved; that claim was wrong and was withdrawn, and the owner has since confirmed the rule above instead. What the owner did NOT decide, and what remains PENDING, is the kiosk's outside-window fallback (below): it is kept exactly as it always was and must not be described as approved.* The rule is defined in exactly one place (`occurrenceWindow(startsAt, durationMinutes)`, `check-in-window.ts`; `durationMinutes` is a required field of the timing shape, so a caller that forgot it fails to compile) and read by automatic matching, by the validation of an explicit selection, by the portal's list and by the moment the page refreshes itself, so what the screen calls "open" is exactly what the server accepts (tests pin that at every boundary, with 60-, 90- and 30-minute classes, overlapping windows and midnight crossings). The schedule editor caps a class at 600 minutes, so the longest window (30 + 600 + 30 minutes) still only ever reaches the calendar day before or after the class's own day; the yesterday / today / tomorrow candidate search is therefore sufficient, and a 600-minute class from Monday 22:00 is tested (open until Tuesday 08:30, filed under Monday). **Overlaps are expected:** back-to-back hourly classes now overlap for an hour. A valid explicit selection always wins (it is validated on the chosen class's own window and never replaced by a different match); with **no** selection the deterministic automatic rule is unchanged - the occurrence whose scheduled start is nearest to the check-in instant, ties to the earlier start, then to the lowest id - and it returns the same class for the same instant in any input order. **Superseded:** any earlier wording that the portal's "check in" button checks in to whatever class matches now, or saves an unmatched tap on a day with no classes. **Kept, deliberately, and still an OPEN OWNER DECISION (not approved):** on the attended kiosk a student can still pick any of today's classes outside its window, and a tap on a day with no classes is saved unmatched for staff to review on the "Marcajes de hoy" table; the portal has no such fallback, so it never shows "closed" while accepting a late selection. The one-attendance-per-day and promotion-reset rules are unchanged: a second class on the same day is still one qualifying day.

**An explicit selection is validated first, in the shared core.** `performCheckIn` takes `pickedClassSessionId` together with a required `pickPolicy` (no default): `OPEN_ONLY` (the portal) accepts only an active class of the student's own academy that is open right now - an unknown, inactive, other-academy or other-organization id is `invalid_class`, a real class outside its window is `class_not_open`, and nothing is written; `TODAY_ANY` (the kiosk) accepts any of today's active classes, exactly as before. A valid selection is never overridden by a different nearest-time match, is filed under the occurrence's own Costa Rica day (so a 00:10 class opened at 23:40 is a Tuesday attendance), and is recorded `STUDENT_PICKED`. Automatic matching for a request with no selection is unchanged (see the overlap rule above). The UI is not the boundary: the portal action accepts only `classSessionId`, requires it, and the core re-validates it.

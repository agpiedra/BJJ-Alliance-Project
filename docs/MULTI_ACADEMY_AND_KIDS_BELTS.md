...
**Found, and now scheduled as B5 — before launch, not after.**
...
|---|---|---|
| **No way to create a second location** (`academy.create` runs only in `approveOrganization` — exactly one academy, named after the city — and in the seed; found in revision 33) | Alliance has **two** locations, Escazú and Escalante. Registering Alliance through the real flow yields one academy and there is no page to add the other. Any customer with more than one location is in the same position. | **Alliance's launch.** Not "a second customer will need it" — the first customer cannot be set up without it. Its own PR (B3): add a location (name, address, kiosk token shown once, default payment plan). |
...
| **Archiving a student is permanent, and the dialog claims it isn't** (found in revision 37; **B5, before launch**) | The archive confirmation says "This can be reversed by editing status later." `status` is deliberately not editable, so nothing reverses it — and since B0, archiving also switches off the student's portal access, so there is no route back at all. A student taking a break and returning is the normal rhythm of a jiu-jitsu academy, so directors will archive people they expect to bring back. | **Alliance's launch** — small, but not after it. B5: an un-archive that goes through `grantStudentMembership`; until it ships the dialog must stop promising it. |
| **A student who logs in before approval sees "not linked to any organization"** (found in revision 37; **B5, before launch**) | Reads as an error. A new student who signs up and tries to log in before staff approve them will message the academy, and the academy will message the platform admin. | **Alliance's launch**, in the same small PR: say they are awaiting approval. |
...
| **A student account cannot also be staff** (found in revision 35, **reclassified as a blocker in revision 36**; a consequence of the global `User.role`, which gates the whole route tree — `/dashboard`, `/students`, `/admin` require a staff role, `/portal` requires `STUDENT`) | An Owner who invites an address that already belongs to a *student's* login is refused (`studentAccount`), because a single account cannot be both: whichever role `User.role` holds, one side of the app would send them to the login page. In a jiu-jitsu academy **every instructor is a student** — the head coach trains there, the purple belt covering Tuesdays trains there, and Alliance's whole coaching staff already exists as student records with student logins — so this is hit at every staff invitation, not the first. The second-email workaround makes it permanent: one person, two identities, checking in for their own training under one and taking attendance under the other, belt progress in one and teaching in the other. | **Alliance's launch** — beside B3. Its own PR (B4), proposed before it is built because it touches authentication: stop gating the route trees on the global role and gate on the membership instead, and let a person who both teaches and trains use both sides of the app from one account. |
...
**Read together:** four items above block Alliance's own launch outright — the Resend
sending domain, and (found by driving the product as a registered owner or a registered
student instead of a seeded user) **no way to create a second location** (B3, revision 33),
**a student account cannot also be staff** (B4, found in revision 35 and reclassified from
"will be hit" to a blocker in revision 36), and **no real student could reach the portal**
(B0, found in revision 36 and closed by revision 37 — it is counted here because it was
the most broken thing in the product, and is fixed only once its PR merges). **No staff
management** was closed by revision 35. Two small items — **B5**: an un-archive, and an "awaiting approval" message for a student who logs in before approval — are also scheduled before launch, not after. The version of this paragraph before revision 33
...
not make for you. Everything else either cannot affect Alliance at all (single-timezone/DST, given

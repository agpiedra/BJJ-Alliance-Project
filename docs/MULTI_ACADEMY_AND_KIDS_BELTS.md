...
**Two accountings coexist on purpose, and an existing organization is moved only through a reviewed step.** `PromotionConfig.stripeAccounting` is `CUMULATIVE` (the old rule, the default, so deploying changes nothing) or `PER_INTERVAL` (the decided rule; new organizations start on it). `pnpm promotion:accounting report --org=<slug>` is read-only and lists who is affected, above all students eligible today who will read 0; `activate ... --report=<id> --apply` refuses unless the report is still true and then flips both tracks, records the tracking baseline for every student and completes the black-belt catalog in one audited transaction. It never edits attendance, promotions, credits or belt dates.

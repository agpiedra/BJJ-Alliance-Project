...
- **Staff resolution.** The Kiosco page lists each academy's pending evidence: the student, the time the tablet reported (in Costa Rica, marked "not verified" or "plausible", or "no valid time" with the raw text), the class it says was chosen, why it was kept and when it was received. A coach of that academy (ADMIN, DIRECTOR or INSTRUCTOR, inside their scope) **records it on the original day**: the day starts as the claimed day when that could be read and is EMPTY when it could not (a person must choose it - an unverified date is never adopted silently); the class must be an active class of that academy scheduled on that weekday; the day cannot be in the future. The attendance is stamped `STAFF_CORRECTED`, `source: KIOSK`, with the coach as author; its `occurredAt` is the claimed instant when that falls on the chosen day, otherwise the class's own start that day - never the replay time. Recording is atomic and single-use (a second click or a second coach gets `notPending` and writes nothing; a class the student already attended that day is `alreadyRecorded` and everything rolls back), and audited. Or the coach **sets it aside** with a reason (3-500 characters); it stays in the database marked with who, when and why. Evidence is never deleted. Once recorded it is an ordinary attendance: one counted attendance per Costa Rica day, and promotion judged by its own `occurredAt` against the interval.

...
**Offline recovery - a separate path from new online check-ins (owner-approved).** A queued tap is an attendance that ALREADY HAPPENED while the tablet had no connection; refusing it would lose it, so it is never refused for its class and never discarded. The server treats a request carrying `queuedAt` as a replay and evaluates it **at its original instant** (never at replay time): a selection recorded with the entry (the student had already chosen from the picker when the connection dropped) is honored if that class was open then; **exactly one** eligible class and no selection -> that class (`AUTO`); **zero** eligible classes (including a legacy queued tap that cannot be resolved), **several** eligible classes with no recorded selection, or a recorded selection that was not valid then -> **retained as UNMATCHED for staff review** on the Kiosco page, with **no promotion contribution** (only a class-attributed attendance can contribute; one class-less attendance per student per day, enforced by the database). An instant that cannot be verified (malformed, in the future, or older than 12 hours) is still kept - at the server clock - but is NOT used to pick a class, so it is retained UNMATCHED rather than attributed to whatever class happens to be open now. The kiosk queue carries the class a student chose before the connection dropped; entries already queued before this change replay unchanged and are evaluated by the same rules. A replay never returns `no_open_class`, `class_selection_required`, `class_not_open` or `invalid_class` (tested); if an older server somehow did, the client logs it, counts it as dropped and raises the persistent "sync dropped" banner for staff - never silently. A wrong code is still `invalid_code`. Known limit: the route cannot tell a genuine replay from a request that merely carries a recent `queuedAt`; that requires the kiosk's device token, which students do not have, and the worst case is one unmatched attendance for staff to review.

**Portal.** The single "check in" button became today's classes for the student's own academy: time range, real modality (Gi, No-Gi, Striking, Kids, Open Mat, Competition - never relabelled), a note when the class does not count toward promotion, and an honest state - open (with the button), already checked in (a voided check-in does not count), opens at HH:mm, or closed; a day with no classes says so. Days and boundaries are Luxon with an explicit America/Costa_Rica zone. After a successful check-in the page data is refreshed in place, so the row, the progress card and the history update without a full reload.

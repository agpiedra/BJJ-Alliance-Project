...
**The availability rule - PROPOSED, provisional until the owner confirms it specifically: one shared rule, the fallback kept on the kiosk.** *Status: this rule (start -30 to +30 minutes, and the kiosk fallback kept) was proposed by the engineer and the recommended options were selected in the working session's decision prompt on 2026-09-24; that is NOT the academy owner's confirmation, and it must not be read as one. The separate decision to keep the user-facing term "attendances" is about terminology only and says nothing about these timing rules. Confirmation of the timing rules is pending; if the owner chooses differently (for example, keeping a class open until it ends), only `occurrenceWindow` and the copy that states the rule change.* A class is OPEN from 30 minutes before its scheduled start until 30 minutes after it, inclusive at both ends, duration ignored - unchanged from section 5 of the original spec, but now defined in exactly one place (`occurrenceWindow`, `check-in-window.ts`) and read by automatic matching, by the validation of an explicit selection and by the portal's list, so what the screen calls "open" is exactly what the server accepts (a test pins that at the window boundaries). **Superseded:** any earlier wording that the portal's "check in" button checks in to whatever class matches now, or saves an unmatched tap on a day with no classes. **Kept, deliberately:** on the attended kiosk a student can still pick any of today's classes outside its window, and a tap on a day with no classes is saved unmatched for staff to review on the "Marcajes de hoy" table; the portal has no such fallback, so it never shows "closed" while accepting a late selection.

**An explicit selection is validated first, in the shared core.** `performCheckIn` takes `pickedClassSessionId` together with a required `pickPolicy` (no default): `OPEN_ONLY` (the portal) accepts only an active class of the student's own academy that is open right now - an unknown, inactive, other-academy or other-organization id is `invalid_class`, a real class outside its window is `class_not_open`, and nothing is written; `TODAY_ANY` (the kiosk) accepts any of today's active classes, exactly as before. A valid selection is never overridden by a different nearest-time match, is filed under the occurrence's own Costa Rica day (so a 00:10 class opened at 23:40 is a Tuesday attendance), and is recorded `STUDENT_PICKED`. Automatic matching for a request with no selection is unchanged. The UI is not the boundary: the portal action accepts only `classSessionId`, requires it, and the core re-validates it.

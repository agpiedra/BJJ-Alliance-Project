...
**As of revision 42 (C1), the launch-blocking list is empty of engineering work.** What
stands between here and Alliance using the product is a name, a domain, and account setup —
the platform's own name (still `[Platform name TBD]` throughout the UI), the Resend sending
domain named above, and standing up the Vercel/Supabase/Healthchecks.io accounts the
runbook already describes — none of it code, all of it yours to do.

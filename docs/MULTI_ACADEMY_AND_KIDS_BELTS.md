...
**Revision:** 47 — MATROOM Phase 1 visual foundation (the design authority is `design/matroom/`; see "What changed in revision 47"); 46 — the student portal checks in to an EXPLICIT class from today's list and the shared core validates an explicit selection first (ONE window rule for the portal AND the kiosk, per class: start minus 30 minutes to the class's scheduled end plus 30 minutes, inclusive; the kiosk's outside-window fallback REMOVED; the kiosk asks which class BEFORE writing when several are open; offline recovery documented separately; every one of these owner-approved), class-less check-ins are unique per student per day in the database, and attendance history is a complete, paginated section with a defined total. Revision 45 — promotion progress follows the academy's decided rules: one qualifying attendance per Costa Rica day, progress resets to 0 after every award (no carryover, no head-start credits), a time-based black belt with a different interval per degree, and no automatic promotions anywhere (enforced in the award writer, the config validator, the database and by removing the scheduled path). Revision 44 — logo upload/removal are ADMIN/DIRECTOR (were silently ADMIN-only, arbitrarily narrower than the settings page itself), storage failures are a classified never-thrown result with distinct user-facing copy, and a masked-fixture defect in this project's own test suite (a "director can remove a logo" test that instantiated an ADMIN) is fixed and logged. Revision 43 — C2 (the /platform observability views) and Sentry are designed, approved, and deliberately deferred until Alliance has real usage and real errors to build them against — recorded so neither is mistaken for forgotten work; the engineering that blocks launch is now complete, and what remains (name, domain, accounts, deploy, bootstrap, terms of service) is entirely the user's own next steps. Revision 42 — C1: both scheduled jobs now write a `JobRun` history (real sent/failed/skipped counts, never masked as `ok: true` when something failed) and ping a Healthchecks.io dead-man's switch, finalizing the row BEFORE pinging so a killed process is a false alarm rather than silence; `GET /api/health` (public, database-only, briefly cached) and `GET /api/health/jobs` (behind `CRON_SECRET`, per-job staleness/stuck detection) give the platform its first real observability. Revision 41 — the tenant gates can no longer be permissive by omission: the role list is required at both `requireTenantContext` and `resolveActionContext`, every call names its roles as a literal, and the finding is recorded as its own pattern (a guard that holds only because another layer covers for it — the tally of "built it, never wired it" stays at fourteen); the sibling audit found `regenerateStudentCode` with the same shape (not exploitable, and fixed); a demoted-while-logged-in person is told, not sent to a bare 404; approving a student refreshes the page; and the join page stops telling a signed-in person to sign in. Revision 40 — B4: access is membership, not the global role — the middleware gates on a `{ staff, portal }` claim derived from the database, never on `User.role`; a stale claim is refreshed from the database (never a silent refusal, never "log out"), a missing one fails closed; one account reaches both the staff app and the portal (promotion in place at invitation acceptance, "Student only" to demote); and a real hole the new tests found — `requireTenantContext()` with no role list admitted students — is closed. Revision 39 — B5: an archived student can be restored, back to the status they had, from a **stored** `Student.statusBeforeArchive` — never read from the audit log; a pending applicant who logs in is told their registration is awaiting approval; the archive dialog stops promising what did not exist; and the kiosk "NaN" found in revision 38 is fixed (#47). Revision 38 — B3: an Owner can add a location (`/admin/locations`, add-only), its kiosk token is shown once and provably opens that location's kiosk, "Both locations" becomes "All locations" and nothing offers a choice at exactly one location, a member without a page's role gets a 404 instead of a raw 500, and the branding, notification and onboarding actions name the organization they act on. Found while verifying it in a real browser and **not fixed here**: every real kiosk check-in shows "1 / NaN" — the fourteenth "built it, never wired it" instance, launch-blocking. Revision 37 — B0: real students can reach the student portal. A self-registered, staff-approved student logged in to "No organization access" because nothing ever created their `STUDENT` membership; approval now does, archiving switches it off, and it can never overwrite or disable a staff membership held by the same person. The thirteenth "built it, never wired it" instance, and by user count the worst. Revision 36 — the student/staff account conflict reclassified from "will be hit at the first staff invitation" to **launch-blocking** (B4): in a jiu-jitsu academy every instructor is a student, and the second-email workaround makes the split permanent; the "built it, never wired it" tally extended to twelve. Revision 35 — staff management, Owner-only (`/admin/staff`): invite by email with a role and academies, edit, deactivate/reactivate *per organization*, resend/revoke invitations, and a copyable link; with the two defects it was blocked on fixed test-first — `acceptInvitation` overwrote an existing account's password, and deactivation was global (`User.active`), so removing someone from one organization locked them out of every organization they belong to. Revision 34 — payment plans become manageable (create, price, edit, deactivate — never delete), every organization prices in a currency (colones or dollars) and every payment carries the currency it was recorded in, and a newly approved academy starts with a monthly plan named in its own language; verifying as a registered owner in a real browser found one more bug that predates this revision (a payment saved with the method left on its default always failed). Revision 33 — verifying as a genuinely REGISTERED owner (register, approve, accept, log in) instead of a seeded account found two launch-blocking bugs invisible for five phases: every customer organization's owner was a `DIRECTOR` with no academies and could see nothing (fixed: approval grants `ADMIN`; the proposed ADMIN/DIRECTOR merge was cancelled — they are different roles, now labelled "Owner"/"Location director"), and the public registration form rejected every real browser submission since #24 (framework-injected `$ACTION_*` fields tripped `z.strictObject`; fixed, with a structural guard); the same verification showed no UI adds staff, assigns academies, or creates a second location, now launch-blocking checklist items. Revision 32 — SSL to the database decided and built (full verification against the provider's CA, supplied as a base64 env var, with a documented, self-announcing `no-verify` fallback that has an owner and an exit condition), after checking HOW TLS is actually configured on the `pg` driver-adapter path and finding it is neither where the connection string suggests nor where the Prisma CLI does it; and the "watch for it" first-deploy advice replaced by a runbook step that runs a real verification script (`pnpm verify:pooling`) before anyone uses the app. Revision 31 — the pooling fix revision 30 *described* is now actually implemented, and verification against the real driver-adapter path showed most of what revision 30 said about it was wrong: `?pgbouncer=true` is a Prisma-engine parameter and does nothing under `PrismaPg`; the real transaction-pooling hazard (named prepared statements) is avoided by construction and now pinned by a test; migrations go direct (`DIRECT_URL`, via `prisma7.config.ts`) because a pooled `migrate deploy` leaks its advisory lock and breaks the *next* deploy, not because DDL is unsupported; `max: 1` was bad advice; SSL to the pooler is an open decision. Revision 30 — the deployment gap named in the launch checklist closed: `docs/DEPLOYMENT_RUNBOOK.md` written, deciding the database/hosting provider Phase 0 deliberately left open (Vercel Pro + Supabase Pro, $45/month, chosen over a cheaper Railway-consolidated stack because `vercel.json` already exists and this pairing is the best-documented failure surface for the stack, not because it's cheapest — the tradeoff was made explicitly, not defaulted into); a real, unaddressed connection-pooling gap found in `src/lib/prisma/unscoped.ts` (no pooler awareness at all — fine for dev's single long-lived process, a textbook serverless-Postgres exhaustion risk in production); five more production-only failure modes named and ranked; README's own bootstrap section corrected after tracing a circularity in its previous version (register-then-approve has no path to the very first approver) that hadn't been caught until verified line-by-line against `approve-organization.ts`. Revision 29: found and fixed a pre-existing, already-shipped bug along the way (the director's own configurable brand color was the default data-series color on two existing charts, including `class-popularity-panel.tsx`, making a pale brand pick nearly invisible) by changing `BarList`'s own default rather than patching each call site; new fixed, non-brand-configurable `--data`/`--data-muted` tokens added with measured (not eyeballed) WCAG contrast ratios in both themes; the new chart shares its counting logic with `getClassPopularity` through two extracted helpers rather than a second query; a real gap found writing the operations doc (no panel button to resend an invitation for an ACTIVE organization) fixed in the same PR. Revision 28: Phase 7 re-scoped against what actually already existed (most of it, built continuously since Phase 1) rather than built to a stale checklist; `docs/MULTI_ACADEMY_OPERATIONS.md` written for Alexis as an operator, not an architecture summary; `docs/REDESIGN_BRIEF.md` audited and kept (implemented, cited by ~50 files, nothing to fold forward) rather than assumed stale; a launch checklist added naming every deliberately-deferred item with whether it blocks Alliance's own launch or only a future customer's. Revision 27: the timezone finding reframed and sized: not "a DST gap" but a single-timezone platform presenting itself as multi-tenant, with `Organization.timezone` a column no scheduling code reads; estimated at phase scale (core functions, context plumbing, 21 call sites, 19 test files, one real DST-edge-case design decision), not fixed yet — scheduled as a known-cost decision, not left to be discovered by a customer. Revision 26: Phase 6 billing shipped as its own PR; two bugs found only by live-browser verification (a bound-action form field silently rejected by `z.strictObject`, an off-by-one invoice deadline from a UTC/org-timezone mismatch); every `vitest.*.config.ts` now forces a deliberately hostile test-runner timezone so a test can no longer pass by agreeing with the machine it happens to run on. Revision 25: a dependency's own console output flagged, verified, and silenced as a prompt-injection surface, not because anything was actually injected that time. Revision 24: the real runtime backstop built and attached to the base client; see below for the actual blast radius this revealed. Revision 23: the Phase 1 "runtime backstop" claim corrected; it was never attached to the base client. Revision 22: the login/tenant-context conflation closed. Revision 21: repository-reviewed. Phases 1 and 2 supersede the first draft entirely; suspended-organization policy and platform billing (manual invoices, snapshotted grace, episode-scoped review) fully specified; branding moved to a first-login onboarding wizard; realistic belt rendering, student-page promotions, full sidebar palette, the attendance-by-class chart added; Phase 0 rebuilt around a clean dev database plus a deterministic seed, and the Phase 1 review decisions settled in Appendix C.

---

## What changed in revision 47 — MATROOM Phase 1: visual foundation (no behavior change)

Presentation only. No route, server action, domain function, permission, migration, or accounting rule changed; both promotion accountings render exactly as before.

- **Design authority:** `design/matroom/` (`DESIGN.md`, `tokens.css`, the approved references with SHA-256 manifest, the frozen Phase 1 preview, the corrected proposal). It supersedes `docs/REDESIGN_BRIEF.md`'s and `design/alliance-mock.html`'s visual direction (both now carry a note); their functional requirements stand. The requirements recorded there for the later page phases (compact mobile metrics, uncramped promotion rows, meaningful chart labels and values, correct singular/plural messages) are binding on those phases.
- **Identity:** `PLATFORM_NAME` is `MATROOM` (resolves the launch-list item "the product itself has no chosen name"), a code-native mark and wordmark (`matroom-mark.tsx`), new favicon, app icons, manifest and theme colours. A tenant's own logo or initials still replaces it wherever a tenant is in scope.
- **Tokens and type:** forest-and-ivory palette in both themes with a 3:1 control-boundary token, a status set and sidebar tokens; IBM Plex Sans for all UI and headings (Archivo retired); `--font-display` (Georgia stack) defined but unused until the landing phase.
- **Tenant colours are preserved, not rewritten.** Stored `OrganizationBranding` values reach the UI exactly as before; `BrandingScope` adds three presentation tokens derived at render time (a 1px edge on a tenant's buttons where the fill is under 3:1, a 3:1-adjusted data colour, a per-tenant sidebar-muted colour). Open item: the schema defaults (`#FACC15`, `#111827`) mean every organization has stored colours, so MATROOM forest shows only where no tenant is in scope; making it the default for organizations that never chose a colour is a separate decision plus a migration.
- **Shared controls:** `Button` (loading state, 44px coarse-pointer targets, boundary and disabled states), `Input`, `Card`, `ProgressToNextGrade` (a real progressbar), the sidebar primitives.
- **Separate, unchanged:** the registration findings (concurrent same-slug and same-email submissions, a stale availability status) are functional and outside this revision. No email uniqueness constraint was added; the intended organization/contact-email policy has to be confirmed first.

---

## What changed in revision 46 — explicit class check-in and a complete attendance history (PR 3)

**Verified against the merged code first (the standing rule: a brief is not ground truth).** Every claim in the original brief held: the portal discarded the class picklist and sent no class; an explicitly picked class was consulted only when no window matched (**reproduced**: with classes at 18:00 and 19:00 a student picking 19:00 at exactly 18:30 was recorded on 18:00, `matchSource = AUTO`); the automatic window is start minus 30 to start plus 30 minutes, inclusive, duration ignored; the kiosk's fallback lets a student pick any of today's classes at any hour; the class-less duplicate guard was an application pre-check (**reproduced**: eight concurrent taps wrote eight rows); the portal history was capped at 50 with no way to reach older records and no total. A defect the brief did not list was found in the same path: a picked class that does not count toward promotion was reported as "already counted today". **What PR #57 already resolved and this PR relies on:** one qualifying attendance per Costa Rica day and the truthful per-check-in outcome messages, unmatched taps never counting toward progress, voided rows ignored everywhere, and the per-class-occurrence uniqueness being a partial index over valid rows.

**The availability rule - APPROVED by the owner, for every class and both channels: one rule, computed per class.** `opensAt` = the class's scheduled start minus 30 minutes; `closesAt` = its scheduled start plus **its own configured duration** plus 30 minutes; **both boundaries inclusive**; America/Costa_Rica (UTC-6, no daylight saving) and each class's own duration. There is **no shared evening window and no fixed cutoff** (no "9 pm"). Examples: an 18:00-19:00 class is open 17:30-19:30; a 19:00-20:30 class 18:30-21:00; an 18:30-19:30 class 18:00-20:00. *History, so this is not misread: an earlier draft of this PR proposed start minus 30 to start plus 30 minutes and described it as owner-approved; that claim was wrong and was withdrawn, and the owner then confirmed the rule above. The owner has since ALSO approved the two kiosk rules below (no outside-window fallback; ask before writing when several classes are open); nothing in this section is provisional any more.* The rule is defined in exactly one place (`occurrenceWindow(startsAt, durationMinutes)`, `check-in-window.ts`; `durationMinutes` is a required field of the timing shape, so a caller that forgot it fails to compile) and read by check-in validation (portal and kiosk), by the portal's list, by the kiosk's picker and by the moment the page refreshes itself, so what the screen calls "open" is exactly what the server accepts (tests pin that at every boundary, with 60-, 90- and 30-minute classes, overlapping windows and midnight crossings). The schedule editor caps a class at 600 minutes, so the longest window (30 + 600 + 30 minutes) still only ever reaches the calendar day before or after the class's own day; the yesterday / today / tomorrow candidate search is therefore sufficient, and a 600-minute class from Monday 22:00 is tested (open until Tuesday 08:30, filed under Monday). **Overlaps are expected:** back-to-back hourly classes overlap for an hour.

**No outside-window fallback (owner-approved).** The attended kiosk used to let a student pick any of today's classes at any hour, and used to save a tap on a day with no open class as an UNMATCHED attendance. **Both are removed.** The kiosk validates a selection exactly as the portal does. When no class is open, a NEW online attempt is refused - `no_open_class`, shown as "check-in is not available right now: no class is open, ask a coach to record your attendance" - and **nothing is written; in particular no unmatched attendance is ever created from a new online attempt.** A coach records the attendance from the Kiosco page. (`pickPolicy` no longer exists: there is one policy.) **Superseded:** the earlier wording that the portal's "check in" button checks in to whatever class matches now, that the kiosk picks the class whose start is NEAREST the tap when several windows match (a tie going to the earlier start), and that the kiosk offers "today's classes" when nothing matched. The one-attendance-per-day and promotion-reset rules are unchanged: a second class on the same day is still one qualifying day.

**Which class, when windows overlap (owner-approved).** `performCheckIn` lists the classes whose window contains the instant (`openOccurrences`, ordered by scheduled start then id). With no selection: **zero** open -> `no_open_class`, nothing written; **exactly one** open -> selected automatically (`AUTO`); **several** open -> `class_selection_required`, which carries the open classes (id, name, scheduled range, real class type) and writes **nothing**: the kiosk shows a picker, and only the student's choice records the attendance. Cancelling the picker, or abandoning it (it returns to the keypad after 30 seconds), creates no attendance. A selection (`pickedClassSessionId`) is validated again on submission, against fresh queries, so the UI is never the boundary: an id that is not an active class of this academy (unknown, inactive, another academy or organization) is `invalid_class`; a real class that is not open at that instant is `class_not_open`, and carries the FRESH open classes (the kiosk explains that the class just closed and refreshes its choices; with none left it says check-in is unavailable); a valid selection is recorded `STUDENT_PICKED`, filed under the occurrence's own Costa Rica day (a 00:10 class opened at 23:40 is a Tuesday attendance), and **always wins**. The portal sends the class its list shows as open, so it never meets the picker; its action accepts only `classSessionId`, requires it, and the core re-validates it. Refusals are outcomes of a VALID code and never count toward the kiosk's wrong-code lockout; duplicate submissions (a double-tap on the picker) write one row and the rest are `already_checked_in`.

**Corrections (owner-approved: the same window).** The kiosk's student-facing "not this class" link (`POST /api/kiosk/reassign`) obeys the check-in window too, so a correction can never produce an attendance the check-in itself would have refused: the choices are the OTHER classes that were open at the instant the attendance was recorded, and the submitted class is validated again against that same instant (`classNotOpen` otherwise). It is judged at the RECORD's instant, not when the student taps the link, so a correction made a moment later is neither wrongly refused nor able to reach a class that was closed when the student tapped. The record follows the chosen class's own Costa Rica day (correcting a 23:50 tap from a class that started at 22:00 to one that starts at 00:10 moves it to the next day). The link is offered only when another class was open at that instant. The type of the reassignment options makes the window impossible to forget: a student correction REQUIRES the instant, a coach's does not. A coach's "Cambiar" on the Kiosco page is deliberately unchanged and not bound by the window: a coach records what actually happened.

**Offline recovery - a separate path from new online check-ins (owner-approved; corrected).** A queued tap is an attendance that ALREADY HAPPENED while the tablet had no connection, so it is never refused for its class and never discarded. Each queued event carries an id the device generated when it queued it. The server treats a request carrying `queuedAt` as a replay and identifies the EVENT: `id:<the device's event id>`, or - for entries queued before ids existed - `legacy:<the claimed timestamp>`, or a random key when even that is unusable. It evaluates the replay at its original instant, but only when that instant is verifiable (a number that is not in the future and not older than 12 hours):
- a recorded selection that was open then -> a real attendance (`STUDENT_PICKED`); exactly ONE eligible class and no selection -> a real attendance (`AUTO`); both at the ORIGINAL instant and the class's own day;
- **everything else** - zero eligible classes, several without a recorded selection, a recorded selection that was not open then, or an instant that cannot be verified (malformed, in the future, older than 12 hours) - is **not turned into an attendance**. It is kept as untrusted **evidence** (`QueuedCheckIn`): the timestamp exactly as claimed (the raw text, the parsed instant when it was a valid number, and whether it could be verified), the selection exactly as sent, why it was kept, and when the server received it. Evidence takes no day of the ledger, counts for nothing (not promotion, not the lifetime total, not the history), and never collides with another event. *Defect fixed here: the earlier design stored these as class-less UNMATCHED attendances dated the REPLAY day, so two distinct stale taps replayed on one day collided on the one-class-less-attendance-per-day index, the first lost its original timestamp and selected class, and the second was answered `already_checked_in`, which the device queue treats as final and deletes.* Evidence is idempotent per event: the same event again is a 200 with `duplicate: true` (a retry), and a different event is a different record even for the same student and day; a 200 with `retained: true` is what lets the device delete its copy. `already_checked_in` now only ever means that this student already has an attendance for that class and day - never that some other unresolved event took a slot.
- **Staff resolution.** The Kiosco page lists each academy's pending evidence: the student, the time the tablet reported (in Costa Rica, marked "not verified" or "plausible", or "no valid time" with the raw text), the class it says was chosen, why it was kept and when it was received. A coach of that academy (ADMIN, DIRECTOR or INSTRUCTOR, inside their scope) **records it against the class occurrence**, confirming two separate things: the **class's day** (the attendance's ledger `date`; it starts as the claimed day when that could be read and is EMPTY when it could not - an unverified date is never adopted silently) with a class that is an active class of that academy scheduled on that weekday, and the **time of the tap** (`HH:mm`, Costa Rica; REQUIRED; it starts as the tablet's claimed time when that could be read and is empty otherwise). The two are different on purpose: a Monday 23:50 tap belongs to Tuesday's 00:10 class, so the tap's own calendar day is whichever of the day before / the class's day / the day after puts it inside that occurrence's check-in window (start - 30 min to end + 30 min, inclusive; at most one does). **The final instant is what is validated**: a time that fits no occurrence of that class - or whose exact recorded instant (the claim keeps its seconds and milliseconds, so a claim 1 ms after the window closes is out even when its `HH:mm` is in) is outside the window - is `timeOutsideClass`, and an instant in the future is `futureTime` (the evidence stays pending and can be recorded once that moment has passed). **Nothing is ever derived from the class start** - an invented instant could sit on the other side of a promotion and change what counts for the new interval - so when the actual time cannot be established the coach sets the evidence aside instead. The attendance is stamped `STAFF_CORRECTED`, `source: KIOSK`, with the coach as author; its `occurredAt` is the tablet's claimed instant, kept to the second, when the confirmed time is that very minute, otherwise exactly the time the coach entered - never the replay time - and the audit entry records the confirmed time and whether the instant was the claim (`CLAIMED`) or the coach's (`STAFF_ENTERED`); the original claim stays on the evidence row. Because promotion is judged by `occurredAt`, a Monday 23:50 tap recorded after a promotion at Monday 23:55 belongs to the interval before it and adds nothing to the next. Recording is atomic and single-use (a second click or a second coach gets `notPending` and writes nothing; a class the student already attended that day is `alreadyRecorded` and everything rolls back), and audited. Or the coach **sets it aside** with a reason (3-500 characters); it stays in the database marked with who, when and why. Evidence is never deleted. Once recorded it is an ordinary attendance: one counted attendance per Costa Rica day, and promotion judged by its own `occurredAt` against the interval.
- The device queue sends the event id with every replay; a recorded selection travels with the entry; entries queued before this change replay unchanged (keyed by their claimed timestamp) and are evaluated by the same rules. A replay never returns `no_open_class`, `class_selection_required`, `class_not_open` or `invalid_class` (tested); if an older server somehow did, the client logs it, counts it as dropped and raises the persistent "sync dropped" banner for staff - never silently. A wrong code is still `invalid_code`.
- **The trust boundary, stated accurately.** The check-in route is authenticated by the academy's kiosk device token (in the kiosk URL on the tablet; an ADMIN can rotate it) plus a student's code (four digits; five wrong guesses in a minute lock the kiosk out). Whoever holds the token can call the route directly and choose what to send in `queuedAt` and `eventId`; nothing (no signature, no server-issued nonce) ties a `queuedAt` to a real tap. The limits are only: the instant must be within the last 12 hours and not in the future, the class window rule is applied AT that instant, and one attendance per class occurrence and one counted attendance per day still hold. **So the consequence is not merely "one unmatched attendance for staff to review"**: a `queuedAt` within the last 12 hours that falls inside exactly one open class window (or a selection that was open then) is treated as verified and becomes an ATTRIBUTED, promotion-eligible attendance at that claimed instant - including for a class whose window has already closed for live check-ins. A token holder with a valid student code can therefore back-date an attendance by up to 12 hours; a claim outside that bound, or an ambiguous one, can only ever become evidence for staff. Students do not have the token, so the realistic exposure is a leaked tablet link or staff misuse. No mitigation is built here (candidates, if the owner wants one: a server-issued nonce per tablet session, a minimum age for a replay, or signing queued entries on the device).

**A page left open follows the clock.** The portal's class list changes with time - a window opens, a window closes, the Costa Rica day rolls over - so the page must not need a manual reload. The server returns, with the list, the next instant it can change (`nextBoundaryAfter`: every opening at start minus 30 minutes, every closing at the first instant after scheduled end plus 30 minutes - each class with its own duration - including adjacent-day windows that open tonight or are still open after midnight, and 00:00 America/Costa_Rica) and its own clock. The card refreshes the server data at that instant, measured on the SERVER's clock (so a wrong browser clock cannot make it early or late), again when the tab becomes visible or the window regains focus (browsers throttle or suspend timers in background tabs and sleeping devices), and after any refused check-in. This is only a re-read: the buttons still come from the server's list and every check-in is re-validated by the server, so a stale, early or late refresh can never make a check-in succeed. Verified by unit tests with fake timers (before the boundary nothing happens, at it exactly one refresh, browser clock fast and slow, midnight, wake after sleep, unmount), by an integration walk that advances time boundary by boundary through an evening and across midnight without submitting anything (nothing changes before an announced boundary, something changes at it), and in a real browser (see the PR).

**Duplicates.** Attributed attendance keeps its per-occurrence unique index; class-less check-ins now have their own partial unique index (one per student per Costa Rica day, valid rows only, `AttendanceRecord_unmatched_student_date_key`), so concurrent unmatched taps are refused by the database instead of racing an application check (class-less check-ins now arise only from staff and from rows created before this change; a new online attempt never creates one, and a queued replay no longer does either - it becomes review evidence). Concurrent explicit selections of the same class write exactly one row (tested).

**Before applying migration `20260924034500_attendance_unmatched_daily_key` to any database other than the two already checked (the development and test databases, which held no duplicates).** This applies to a database that already has an `AttendanceRecord` table; on a genuinely empty database (no schema yet, such as a first production deploy) the query cannot run (`relation "AttendanceRecord" does not exist`) and there is nothing to preflight - `docs/DEPLOYMENT_RUNBOOK.md`, "Releasing a commit that adds migrations", tells the two cases apart. The migration creates a unique index, so it FAILS if the database already holds two valid class-less check-ins for one student and one day. It changes nothing when it fails, but Prisma then records the migration as failed and blocks later deploys until it is resolved, so run this read-only query first and expect zero rows:
...
If it returns rows, **nothing is deleted or voided automatically, and no script does it for you**: each group is a student's repeated unmatched taps on one day (only the first was ever meant to count). Staff decide, group by group: an ADMIN or DIRECTOR either voids the extra entries with the existing audited void action (a reason is required; the rows stay in the history, marked with who, when and why) or, where a tap was really for a class, reassigns it to that class with "Cambiar" on the Marcajes table. Re-run the query until it returns zero rows, then apply the migration. If a migration was already attempted and failed, resolve it with `prisma migrate resolve --rolled-back 20260924034500_attendance_unmatched_daily_key` after cleaning the duplicates, then deploy again. The query was verified on the test database: with the index dropped and three class-less check-ins planted on one day it returned exactly that group (ordered by time), and it reported nothing once two of the three were voided; the index was then restored and the drift check re-run clean.

**Attendance history.** A dedicated section replaces the inline card: a defined total - **check-ins + staff-added days + other signed adjustments** (the last are legacy rows that are not +1; the academy no longer creates them) - computed over the whole ledger with aggregates and reconciling with the "lifetime attendances" figure; voided entries count for nothing; promotion credit is not attendance and, where an organization still counts it, is shown on its own line. Entries load newest-first in a stable keyset order (occurredAt, id) with a "show older attendance" control, so a page boundary can neither skip nor repeat records that share an instant and a newly arrived record cannot shift older pages; "showing X of Y" uses the ledger count, never the loaded length. Each entry says what it is (class check-in, check-in with no class, added by staff, adjustment) with its time in Costa Rica. The section is keyboard and screen-reader operable (labelled region, real list, `<time>`, native button, polite announcement, focus to the first new entry). The action that loads more takes no student id: it serves the caller's own linked student only.

**Not changed:** the daily attendance rule, the promotion-reset rules, the kiosk's authentication and rate limiting, and the MATROOM visual design.

## What changed in revision 45 — promotion progress: one attendance per day, reset after every award, a time-based black belt, manual awards only

**The rule the spec called "the reference behavior" was replaced by the academy's own decisions** (confirmed through the owner on 2026-09-23; the questions and the recorded answers are in `docs/PROMOTION_DIRECTOR_QUESTIONNAIRE.md`, the decision register and what was built in `docs/PROMOTION_PROGRESS_PROPOSAL.md`). The earlier cumulative-since-the-belt-anchor rule, "Only belt awards reset the anchor", and Appendix A's "must not regress" are **superseded**; each passage this replaces is marked in place below rather than silently edited.

**What is true now.** A student receives at most **one** qualifying attendance per America/Costa_Rica calendar day, across kiosk, portal and coach-added entries (every row stays in the history; the day's contribution is the earliest qualifying row, derived from the rows so it holds under concurrency, retries and offline replay with no lock). **Progress resets to 0 at every promotion**, stripes/degrees and belts, adults and kids: the exact award instant is saved as the promotion's `awardedAt` and as the new `Student.progressBaselineAt`, the class attended before it belongs to the completed interval, and nothing carries over. Students keep their entered rank and stripes and start at 0 from a **system tracking baseline** recorded separately from the historical belt date; no head-start credits, no arbitrary progress adjustments, and no historical promotion is ever invented. The adult thresholds stay 30/65/75/85 and kids stay 10/10, per stripe or degree and again for the belt. The **black belt is time-based**, not attendance-based: 36, 36, 36, 60, 60, 60 months for degrees 1 to 6 (`BeltRank.stripeIntervalMonths`, a different interval per degree), later degrees unconfigured rather than impossible, and an existing black belt with no known last-award date shows rank and attendance and no due date until it is supplied. **No promotion is automatic, for any academy:** the cron route, the automation module and the job registration are gone, `writeAward` refuses `AUTO`, `updateTrackConfig` refuses turning approval off, and a database CHECK refuses it for every other writer; historical `AUTO` rows stay.

**Two accountings coexist on purpose, and an existing organization is moved only through a reviewed step.** `PromotionConfig.stripeAccounting` is `CUMULATIVE` (the old counting rule, the default: deploying does not change how an existing organization's progress is COUNTED - but it does change other things immediately, listed in the proposal's section 5: automatic promotion and the head-start credit UI are removed, coach attendance entry becomes one day, a mistaken entry can be voided, and every award now records its exact instant) or `PER_INTERVAL` (the decided rule; new organizations start on it). `pnpm promotion:accounting report --org=<slug>` is read-only and lists who is affected, above all students eligible today who will read 0; `activate ... --report=<id> --apply` validates and applies in ONE serializable transaction (an organization advisory lock plus row locks; the report is rebuilt inside it and must equal the approved id, which binds approval to the reported outcomes including each black belt's evaluated state), and then flips only the legacy tracks, records the tracking baseline for their students and completes the black-belt catalog (only when the ADULT track is itself being activated: an already-active ADULT track keeps its own configuration), with one audit row. A voided attendance entry no longer blocks a valid replacement check-in (the per-class-occurrence uniqueness is a partial unique index over valid rows), and a coach-recorded day of a promotion is flagged `historyOnly` on the row so no later void can turn it into a progress contribution. It never edits attendance, promotions, credits or belt dates.

**The award writer changed shape, and the reason matters.** `writeAward` now locks the student row, chooses the award instant while holding the lock (never earlier than one millisecond after the previous boundary, and explicitly **not** the commit time), re-checks eligibility inside the transaction against the evidence as of that instant, and records that evidence in the audit row. A row committed later with an `occurredAt` before the instant belongs to the interval that was closed - late-recorded attendance stays in history and adds nothing toward the next stripe. A correction never restarts progress and never revokes a promotion; a coach explicitly corrects an award.

**Display.** One evaluation path (`evaluateStudentProgress`) and one display model (`buildProgressView`) serve staff, portal, kiosk and analytics; the engine returns the target so no screen rebuilds it as `count + remaining` (which read `42/42` past the threshold). Eligible means eligible for instructor review: full bar, zero remaining, the real count on its own line. Staff also see the threshold-reaching day, recalculated from current records on every read and never stored. "New stripe!" copy is now "Ready for review", because a check-in never awards anything.

**Deliberately not built, so nobody re-proposes it:** carryover and carry corrections, head-start credits, signed progress adjustments, date-only and unknown anchors, reconciliation states, and any automatic promotion. **Still PENDING and the academy's to answer:** the roster of existing black belts (degree and last-award date), (a wrongly recorded attendance day CAN now be voided by an ADMIN/DIRECTOR with a reason - audited, never deleted, the day's contribution recomputed, no promotion revoked; the small open points are instructor self-void and an un-void). **Also to decide before running the activation on a live organization:** students eligible under the old rule that day will read 0 afterwards.

**A brief's premises were checked against the code, per the standing rule, and three were wrong:** the promotion-configuration screen the brief assumed does not exist (`updateTrackConfig` has no production caller), `timeAnchorAt` was documented as reset by every award but no award wrote it, and the TIME preset for a new organization leaves month thresholds unset so it never worked. The first two are corrected by this change; the third is unchanged and recorded as a limitation.
...
**Second correction, found by review AFTER this PR merged: the 15-second deadline stopped applying to a slow error body.** `storageFetch` returned `classifyResponse(response)` bare, without `await`, when a response came back non-2xx. `classifyResponse` reads the response body (`readErrorBody`'s `response.json()`) — still in-flight I/O — but the un-awaited `return` let the surrounding `finally`'s `clearTimeout` run the instant that line was reached, not after the body-read promise settled. For a response whose headers arrived but whose body then stalled (a slow or interrupted stream), the abort timer was already disarmed by the time it would have mattered: nothing would ever call `controller.abort()` again, so the read could hang indefinitely — the deadline covered `fetch()` itself but not the classification work built on top of it. Fixed with an explicit `return await classifyResponse(response)`, which keeps the `try` block (and therefore the timer) alive until classification actually finishes. Regression test: a fake response whose `.json()` doesn't resolve until the same abort signal fires — reproduced the hang against the un-awaited code (the test timed out, as it should) before confirming the fix resolves it.
...
- **Naming `STUDENT` in a gate is how a page or action admits students**, so only the portal gate (`requirePortalContext`, in `context.ts`) and the portal's own student actions may: the self check-in action and "show older attendance" (both act only on the caller's own linked student record, with no student id in their input).
...
Support `ATTENDANCE`, `TIME`, `HYBRID`, and `MANUAL` promotion modes while preserving Alliance's existing cumulative attendance behavior. *[Superseded in revision 45 - see "What changed in revision 45" and `docs/PROMOTION_PROGRESS_PROPOSAL.md`.]* (The cumulative behavior is preserved only as `CUMULATIVE` accounting until an organization is moved to `PER_INTERVAL`.)
...
Alliance counts promotion-relevant attendance since `Student.beltAwardedAt`. *[Superseded in revision 45: under `PER_INTERVAL` progress counts qualifying days (one per Costa Rica day) since `Student.progressBaselineAt`, the last award or the system tracking start; `beltAwardedAt` stays the historical belt date and is not read.]*
...
- cumulative promotion-relevant attendance since the belt anchor; *[Superseded in revision 45 for `PER_INTERVAL`: qualifying days since the last award.]*
...
**ATTENDANCE** — use cumulative belt attendance and the thresholds above. *[Superseded in revision 45 for `PER_INTERVAL`: the qualifying days of the current interval against one threshold; every award resets to 0.]*
...
Keep time anchors separate from cumulative attendance accounting. Adding TIME support must not reset Alliance attendance progress. *[Revision 45: `timeAnchorAt` is the instant of the last promotion for time-based ranks and is written by every award; per-degree intervals live in `BeltRank.stripeIntervalMonths`.]*
...
Stripe awards retain cumulative attendance since the belt anchor. Belt awards reset the belt anchor. Attendance rows are never deleted or decremented by promotion operations. *[Superseded in revision 45 for `PER_INTERVAL`: every award, stripe or belt, resets progress to 0 from its exact instant. The last sentence still holds: attendance rows are never deleted or decremented.]*

### Approval and automation

> **Superseded in revision 45: there are no automatic promotions.** The "When false" branch below is not supported for any organization; `requiresCoachApproval` is always true (database CHECK), and the scheduled path no longer exists. Kept for the record of what was once specified.
...
- the next target (`STRIPE` / `BELT` / `NONE`) and progress toward it — for `ATTENDANCE`, the cumulative count and the threshold with what remains ("47 / 50 · faltan 3") *[revision 45: an eligible student shows a full bar, zero remaining and no overflowing fraction, with the real count on its own line]*; for `TIME`, the due date; for `HYBRID`, both, with the binding one marked; for `MANUAL`, a plain statement that promotion is at the coach's discretion;
...
- *[Revision 45: the automation path described in this and the next three bullets was removed; `writeAward` refuses `AUTO`.]*
...
**Alliance kids rule:** `attendancePerStripe = 10` on every kids rank, `extra belt requirement = 10`. Thresholds are cumulative since the belt anchor, exactly as for adults: degree *n* at `n × 10` attendances; the belt at `maxStripes × 10 + 10`. For solid grey that is degree 11 at 110 and grey-black at 120. *[Superseded in revision 45, academy-confirmed: 10 qualifying days per degree, then 10 more for the belt, and progress resets after every award - so degree 11 needs 10 days and the belt another 10, not 110 and 120.]*
...
- [ ] A kids grey belt at degree 11 shows `nextTarget = BELT` and needs cumulative 120 attendances since the belt anchor. *[Superseded in revision 45: needs 10 qualifying days since the last award.]*
...
- **`getCheckInWindow`/`matchOccurrence`/`startOfOccurrence` (`check-in-window.ts`)** — the check-in window around a class (its scheduled start minus 30 minutes to its scheduled end plus 30 minutes) is computed in CR wall-clock time regardless of the academy's real zone, so a real class's actual local start time falls outside the window the code thinks it's in.
...
- **Not pure plumbing — one real design decision.** A parameter rename would be a day's work; what makes this bigger is that a real DST-observing organization introduces genuine new cases with no existing precedent in this codebase: a class scheduled during the "spring forward" missing hour, or during the ambiguous repeated hour on "fall back." `deadline.ts`'s calendar-day arithmetic likely absorbs this correctly by construction, but the check-in window's wall-clock math and the attendance-date derivation need an actual decision about what happens in those two edge cases, then a test proving it — the same rigor `deadline.ts` got for its own boundary, not assumed by analogy.
...
- The promotion **rule engine is built now**: attendance, time, hybrid and manual — but Alliance's cumulative attendance semantics are the reference behavior and must not regress. *[Superseded in revision 45: the academy decided a different rule (one qualifying day per Costa Rica day, reset after every award); `CUMULATIVE` remains only until an organization is moved by the reviewed activation.]*
- Attendance progress is cumulative since the belt anchor and survives stripe awards. Only belt awards reset the anchor. *[Superseded in revision 45: every award resets progress.]*

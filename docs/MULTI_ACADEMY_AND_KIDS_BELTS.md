...
- **A student account cannot also be staff.** The middleware gates the staff route tree on the global `User.role` in the session, so one account cannot be both a student (`/portal`) and staff (`/dashboard`). The invite is refused with a clear error (`studentAccount`) rather than producing a login that goes nowhere. In a jiu-jitsu academy the black belt who starts teaching is a student first, so this will be hit at the first real staff invitation; the workaround is a second email address, and the real fix — stop gating route trees on the global role — is its own change (launch checklist).
...
| **A student account cannot also be staff** (new in revision 35; a consequence of the global `User.role`, which gates the whole route tree — `/dashboard`, `/students`, `/admin` require a staff role, `/portal` requires `STUDENT`) | An Owner who invites an address that already belongs to a *student's* login is refused (`studentAccount`), because a single account cannot be both: whichever role `User.role` holds, one side of the app would send them to the login page. In a jiu-jitsu academy the black belt who starts teaching is a student first, so this will be hit. | **Neither**, but it will be noticed at the first real staff invitation. The workaround is a second email address for the staff account. The real fix is to stop gating the route trees on the global role (read the membership role instead) — its own change, not sized here. |
...
**Read together:** two items above block Alliance's own launch outright — the Resend
sending domain, and (found in revision 33, by driving the product as a registered owner
instead of a seeded user) **no way to create a second location**. A third, **no staff

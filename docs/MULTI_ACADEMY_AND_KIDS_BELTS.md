...
Run under that hostile zone, the full suite (357 unit + 498 integration tests, after the fix above) passed clean — no further failures surfaced. That is a real result, not a weaker one: it means the rest of this codebase's date-handling code (attendance's `attendanceDateFromZoned`, the scheduling window functions, `resolveInvoiceState`/`graceEndsOn`) already takes its timezone as an explicit parameter or a named constant rather than reading the ambient system zone, so it was already immune to the class of bug the hostile-zone run exists to catch. The one place ambient-system-zone reliance existed was in test-authoring code itself (the two fixes above), not in application code.

**Known untested area, flagged rather than fixed: Costa Rica's own timezone never observes DST, so this codebase has never had to be correct across a DST transition.** `src/lib/scheduling/zone.ts` hardcodes `export const ZONE = "America/Costa_Rica"` and every scheduling/attendance function in that file and in `check-in-window.ts` converts through that one constant — never `Organization.timezone`, which already exists as a column and is already read correctly by `src/lib/billing/deadline.ts` and `src/lib/billing/banner.ts`. `src/lib/notifications/weekly-digest.ts`'s trailing-7-day window (`DateTime.now().setZone(ZONE)`) has the same hardcoded dependency. None of this is wrong for a Costa-Rica-only fleet — it is untested and unbuilt for anything else. The day an organization with a DST-observing timezone (a Chile or Mexico academy, for instance) signs up, three things become live risk at once: `attendanceDateFromZoned`/`toAttendanceDate` (a class's ledger day could shift or double-book across a spring-forward/fall-back boundary), the billing due-date math in `deadline.ts` (already `Organization.timezone`-aware, but never verified against a real DST transition — Luxon's calendar-day arithmetic should handle it correctly by construction, but "should" is not "verified"), and the weekly digest's window boundary. This is deliberately not being fixed now — recorded here so it's found by this doc before it's found by a customer.

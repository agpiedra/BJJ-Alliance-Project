...
9. **Approve it**: `scripts/approve-organization.ts --slug=alliance-cr --approved-by=<your-email>`.
   ...
   your own address, since the function has no way to know you don't need one. **Do not open
   it.** Today `acceptInvitation` overwrites the password of an account that already exists
   (open bug, fixed in the staff-management PR), so following that link would silently replace
   the password from step 7.
